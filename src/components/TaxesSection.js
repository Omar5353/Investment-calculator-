    ...
    <div>
      {annualIncome > 0 && (
        <div className="info-card accent-blue">
          <h3 className="info-title">📋 2024 Tax Bracket Estimate</h3>
          <p className="info-text">
            Based on ${annualIncome.toLocaleString()} annual income, your estimated effective federal tax rate is approximately{' '}
            <strong style={{ color: '#60a5fa' }}>{suggestedFederal}%</strong>. You can enter this in the Federal Tax Rate field below.
          </p>
          <div className="brackets-grid">
            {FEDERAL_BRACKETS_2024.map((b, i) => (
              <div
                key={i}
                className={`bracket-item${annualIncome > b.min ? ' active' : ''}`}
              >
